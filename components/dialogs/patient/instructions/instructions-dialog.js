...
export function InstructionDialog({ isOpen, onClose, refreshData, id, initialData }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (initialData) {
            const response = await putData(`patients/instructions/${initialData?.id}`, payload,
            );
            if (response.success) {
            ...
            const response = await postData(`patients/instructions`, payload, {
                headers: {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[525px]">
                <DialogHeader>
                    <DialogTitle>{initialData ? "تحديث تعليمات" : "إضافة تعليمات"}</DialogTitle>
                    <DialogDescription>
                        {initialData ? "قم بتحديث تعليمات هنا." : "أدخل بيانات تعليمات الجديد هنا."}
                    </DialogDescription>
                </DialogHeader>
                <form className="grid gap-4 py-4">
                    ...
                    <div className="space-y-2">
                        <Label> المحتوى</Label>
                        <Textarea name="content" value={formData.content} onChange={(e) => handleChange("content", e.target.value)} />
                    </div>
