...
export function NoteDialog({ isOpen, onClose, refreshData, id, initialData }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (initialData) {
            const response = await putData(`patients/notes/${initialData?.id}`, payload,
            );
            if (response.success) {
            ...
            const response = await postData(`patients/notes`, payload, {
                headers: {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[525px]">
                <DialogHeader>
                    <DialogTitle>{initialData ? "تحديث ملاحظات" : "إضافة ملاحظات"}</DialogTitle>
                    <DialogDescription>
                        {initialData ? "قم بتحديث ملاحظات هنا." : "أدخل بيانات ملاحظات الجديد هنا."}
                    </DialogDescription>
                </DialogHeader>
