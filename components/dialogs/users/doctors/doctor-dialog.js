...
export function DoctorDialog({ isOpen, onClose, onSave, initialData }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.last_name) return toast.error("اسم العائلة مطلوب") && setIsLoading(false);
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[525px]">
                ...
                <form className="grid gap-4 py-4" onSubmit={handleSubmit}>
                    ...
                    <div className="space-y-2">
                        <Label> التخصص</Label>
                        <Input name="specialization" required value={formData.specialization} onChange={(e) => handleChange("specialization", e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <Label> رقم التصنيف</Label>
                        <Input name="classification_number" required value={formData.classification_number} onChange={(e) => handleChange("classification_number", e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <Label> موقع العيادة  </Label>
                        <Input name="workplace_clinic_location" required value={formData.workplace_clinic_location} onChange={(e) => handleChange("workplace_clinic_location", e.target.value)} />
                    </div>
                    <div className="space-y-2">
